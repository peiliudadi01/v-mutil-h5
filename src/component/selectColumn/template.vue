...
<div class="col-wrapper" :style="getWrapperHeight">
  <ul class="wheel-list" :style="getListTop" ref="wheel">
    <li class="wheel-item" v-for="(item, index) in scrollValues" :key="index" :style="initWheelItemDeg(item.index)">{{item.value.cardName}}</li>
  </ul>
...
export default {
  ...
  computed: {
    scrollValues() {
      ...
      // console.log("数组", result, result.length);
      return result;
    },
    getListTop() {
      return {
